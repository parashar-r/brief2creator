...
def load_dataset(uploaded_file):
    ...
    """
    try:
        # Check file extension (convert to lower-case for safety)
...
st.markdown("### Upload your creator dataset to begin (CSV or Excel format)")
...
# At this point, the dataset is valid. Hide file uploader options if needed.
st.markdown("### Enter your campaign brief below:")

# Text area for the campaign brief
...
if campaign_brief.strip() == "":
    st.info("Please enter a campaign brief to find suitable matches.")
    st.stop()

# Load the embedding model (using BAAI/bge-m3) with LLM-based embeddings
...
# Embed creator bios (once dataset is ready)
with st.spinner("Embedding creator bios..."):
    try:
        ...
        # Save each row's embedding as a list
        creators_df["bio_embedding"] = bio_embeddings.tolist()
    except Exception as e:
        st.error(f"Error embedding bios: {e}")
        st.stop()

# Embed campaign brief with the recommended BGE prompt prefix
...
# Calculate cosine similarity between the campaign brief embedding and all creator bio embeddings
with st.spinner("Calculating similarity scores..."):
    try:
        # Stack creator embeddings into one tensor
        creator_embeddings_tensor = torch.tensor(creators_df["bio_embedding"].tolist())
        # Compute cosine similarity; result shape: [1, num_creators]
...
# Provide filter options on the side based on dataset
